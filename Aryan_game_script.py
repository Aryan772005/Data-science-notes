...
PLAYER_COLOR = (0, 255, 200)   # Neon Cyan
ENEMY_COLOR = (255, 50, 100)   # Neon Pink
TEXT_COLOR = (255, 255, 255)   # White
UI_BG = (40, 40, 60)           # Dark grayish-blue for HUD

# Fonts (Using Trebuchet for a cleaner, modern UI look)
...
# Generate a background starfield [x, y, speed, size]
...
def main():
    ...
    enemy_speed = 5
    spawn_timer = 0
    spawn_delay = 40 # Frames between spawns
    ...
    # The try/finally block prevents Jupyter from crashing
    try:
        while running:
            # 1. Clear screen with background color
            screen.fill(BG_COLOR)

            # 2. Draw Scrolling Starfield Background
            for star in stars:
            ...
            if state == "MENU":
                ...
                for enemy in enemies[:]:
                    ...
                    # Remove enemies that go off screen and add points
                    if enemy.top > HEIGHT:
                        enemies.remove(enemy)
                        score += 1
                        
                        # Dynamic Difficulty: Speed up and spawn faster!
                        if score % 5 == 0:
                            enemy_speed += 0.5
                ...
                # Draw Player (Cyan with white inner border and rounded corners)
                ...
                # Draw Enemies (Pink with lighter pink inner border and rounded corners)
                for enemy in enemies:
                ...
                # Draw In-Game HUD (Semi-transparent top bar)
                ui_bar = pygame.Surface((WIDTH, 50))
                ui_bar.set_alpha(180) # Transparency level
                ui_bar.fill(UI_BG)
        ...
        # Safely closes the window and returns control to Jupyter
        pygame.quit()
